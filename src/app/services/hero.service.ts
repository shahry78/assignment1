...
export class HeroService {
  ...
  getNormalUsers (): Observable<Channel[]> {
  ...
  addChannel (hero: Channel): Observable<Channel> {
    return this.http.post<Hero>(this.channelsUrl, hero, httpOptions).pipe(
      tap((hero: Channel) => this.log(`added hero w/ id=${hero.id}`)),
      catchError(this.handleError<Channel>('addChannel'))
    );
  }

  addNormalUser (hero: NormalUser): Observable<NormalUser> {
    return this.http.post<NormalUser>(this.normalUsersUrl, hero, httpOptions).pipe(
      tap((hero: NormalUser) => this.log(`added hero w/ id=${hero.id}`)),
      catchError(this.handleError<NormalUser>('addNormalUser'))
    );
  }
  ...
  deleteChannel (hero: Channel | number): Observable<Channel> {
    ...
    return this.http.delete<Channel>(url, httpOptions).pipe(
      tap(_ => this.log(`deleted hero id=${id}`)),
  ...
  deleteNormalUser (hero: NormalUser | number): Observable<NormalUser> {
    ...
    return this.http.delete<NormalUser>(url, httpOptions).pipe(
      tap(_ => this.log(`deleted hero id=${id}`)),
      catchError(this.handleError<NormalUser>('deleteNormalUser'))
    );
  }
